...
- 2023.05.27 - Updated Field Derivations Utility query to consolidate logic, normalize foreach lines, and use `| union` command instead of a series of appends.
...
```spl
| union maxtime=900 maxout=1000000 timeout=930
    ...
    | rename eai:acl.* AS acl_*, "*'*'*" AS "***", "*'*" AS "**"
    | foreach alias.* 
        [| eval conf_specific_properties_mv = if( len('<<FIELD>>') > 0, mvappend(conf_specific_properties_mv, "<<FIELD>>::::" . '<<FIELD>>'), conf_specific_properties_mv )]
    ...
    | dedup id 
    | rename eai:acl.* AS acl_*, transform AS transforms_stanza, "*'*'*" AS "***", "*'*" AS "**"
    | fields title, type, attribute, acl_app, acl_owner, acl_sharing, value, overwrite, transforms_stanza 
    | foreach * 
        [| eval <<FIELD>> = if( len('<<FIELD>>') > 0, '<<FIELD>>', null() )] 
    ...
    | foreach *_segment_full 
        [| eval conf_specific_properties_mv = if( len('<<FIELD>>') > 0, mvappend(conf_specific_properties_mv, "<<MATCHSTR>>::::" . '<<FIELD>>'), conf_specific_properties_mv )]
    ...
    | dedup id 
    | rename eai:acl.* AS acl_*, REGEX AS value, "*'*'*" AS "***", "*'*" AS "**"
    | foreach * 
        [| eval <<FIELD>> = if( len('<<FIELD>>') > 0, '<<FIELD>>', null() )] 
